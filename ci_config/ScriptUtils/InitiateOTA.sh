    ...
    do
      ...
      eval "${key}='${value}'"
		...
		if [ $KCI_ANT_MODE = "debug" ]; then
		 #$KCI_PWR_SHELL_EXEC_CMD $KCI_ENABLE_OTA $PARENT_JOB_NAME $binaryname $PARENT_BUILD_NUMBER $WIN_KCI_OTA_TEMP_DIR $PARENT_JOB_NAME ${ws_loc}/$KCI_UI_LOCAL_MODULE_DIR/$KCI_GEN_IPA_TASK_IOS_BINARY_DIR $IPA_NAME ${ws_loc}/$KCI_UI_LOCAL_MODULE_DIR/binaries/android $APK_NAME $JENKINS_BASE_HOME/$KCI_PROPS_DIR $iphonebundleidentifierkey $KCI_UI_APP_ID $CHANNELS $BUILD_DETAILS $KCI_S3_BASE_URL $KCI_S3_SUB_FOLDER_URL $KCI_S3_OTA_URL
