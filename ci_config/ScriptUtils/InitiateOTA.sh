  ...
  if [ -f "$propertyFile" ]; then
    ...
    do
      ...
      eval "${key}='${value}'"
		...
		IPA_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_MAC_SLAVE_MAIN_JOB_NAME\\workspace"
		AND_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_MAC_SLAVE_MAIN_JOB_NAME\\workspace"
		WIN_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_WIN_SLAVE_MAIN_JOB_NAME\\workspace"

		AND_TAB_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_MAC_SLAVE_MAIN_JOB_NAME\\workspace"
		...
		if [ $EXECUTE_PIPELINE = true ]; then
			...
			if [ $BUILD_FOR_IOS_RC_CLIENT = true ]; then
				...
				if [[ ! -z $KONY_WIN_SLAVE_INIT_JOB_NAME ]];then
					...
					AND_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_WIN_SLAVE_MAIN_JOB_NAME\\workspace"
				else
		...
		if [ $EXECUTE_PIPELINE = true ]; then
			...
			if [ $BUILD_FOR_IOS_IPAD_RC_CLIENT = true ]; then
				...
				if [[ ! -z $KONY_WIN_SLAVE_INIT_JOB_NAME ]];then
					echo "Windows job exists. Redirecting android build to Win Slave"
					AND_TAB_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_WIN_SLAVE_MAIN_JOB_NAME\\workspace"
				else
					echo "Windows job does not exists. Redirecting android build to Mac Slave"
					AND_TAB_BINARY_FOLDER="$JENKINS_HOME\\jobs\\$KONY_MAC_SLAVE_MAIN_JOB_NAME\\workspace"
				fi
		...
		OTA_PUBLISH_URL=$KONY_CI_S3_BASE_URL

		#if [ $KONY_CI_ENABLE_S3_PUBLISH = "true" ]; then
		#	OTA_PUBLISH_URL=$KONY_CI_S3_BASE_URL
		#else 
		#	OTA_PUBLISH_URL=$KONY_OTA_SERVER_URL
		#fi
		...
		BUILD_DETAILS="<B>GIT_PATH:-</B>$KONY_UI_SVN_PATH</Br><B>GIT_BRANCH:-</B>$KONY_UI_GIT_BRANCH</Br><B>APPLICATION_ID:-</B>$appid</Br><B>IPHONEBUNDLEKEY:-</B>$iphonebundleidentifierkey</Br><B>IPA_PROVISIONING_PROFILE:-</B>$KONY_CI_GEN_IPA_TASK_PRO_PROFILE_NAME</Br>"
		else
		BUILD_DETAILS="<B>GIT_PATH:-</B>$KONY_UI_SVN_PATH</Br><B>GIT_BRANCH:-</B>$KONY_UI_GIT_BRANCH</Br><B>APPLICATION_ID:-</B>$appid</Br>"
		fi

		#BUILD_DETAILS="<B>GIT_PATH:-</B>$KONY_UI_SVN_PATH</Br><B>GIT_BRANCH:-</B>$KONY_UI_GIT_BRANCH</Br><B>APPLICATION_ID:-</B>$appid</Br><B>IPHONEBUNDLEKEY:-</B>$iphonebundleidentifierkey</Br>"
		...
		echo "KONY_CI_ANT_MODE ::$KONY_CI_ANT_MODE"
		...
		if [ $KONY_CI_ANT_MODE = "debug" ]; then
		 #$KONY_CI_PWR_SHELL_EXEC_CMD $KONY_CI_ENABLE_OTA $PARENT_JOB_NAME $binaryname $PARENT_BUILD_NUMBER $WIN_KONY_OTA_TEMP_DIR $PARENT_JOB_NAME ${ws_loc}/$KONY_UI_LOCAL_MODULE_DIR/$KONY_CI_GEN_IPA_TASK_IOS_BINARY_DIR $IPA_NAME ${ws_loc}/$KONY_UI_LOCAL_MODULE_DIR/binaries/android $APK_NAME $JENKINS_BASE_HOME/$KONY_CI_PROPS_DIR $iphonebundleidentifierkey $KONY_UI_APP_ID $CHANNELS $BUILD_DETAILS $KONY_CI_S3_BASE_URL $KONY_CI_S3_SUB_FOLDER_URL $KONY_CI_S3_OTA_URL
		 echo "Ant mode is ::KONY_CI_ANT_MODE"
		 $KONY_CI_PWR_SHELL_EXEC_CMD $KONY_CI_ENABLE_OTA $PARENT_JOB_NAME $binaryname $binaryname $PARENT_BUILD_NUMBER $WIN_KONY_OTA_TEMP_DIR $PARENT_JOB_NAME "$IPA_BINARY_FOLDER/$KONY_CI_GEN_IPA_TASK_IOS_BINARY_DIR" $IPA_NAME "$IPA_BINARY_FOLDER/$KONY_CI_GEN_IPA_TASK_IOS_IPAD_BINARY_DIR" $IPA_NAME "$AND_BINARY_FOLDER/binaries/android" $APK_NAME "$AND_TAB_BINARY_FOLDER/binaries/tabrcandroid" $APK_NAME "$WIN_BINARY_FOLDER/$KONY_CI_GEN_WINDOWS8_BINARY_DIR" $WIN_NAME "$WIN_BINARY_FOLDER/$KONY_CI_GEN_WINDOWS81_BINARY_DIR" $WIN_NAME "$SCRIPTUTILS_FOLDER" $iphonebundleidentifierkey $KONY_UI_APP_ID $CHANNELS $BUILD_DETAILS $KONY_CI_S3_BASE_URL $KONY_CI_S3_SUB_FOLDER_URL $KONY_CI_S3_OTA_URL $EXECUTE_PIPELINE $BuildMachineOS $ANDROID_TARGET_SLAVE_IN_PIPELINE "$NOTIFICATION_FOLDER"
		else
